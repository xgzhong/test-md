...
// Note DTOs
public record NoteDto(int Id, int? FolderId, string? FolderName, string Title, string Content, bool IsShared, string? ShareToken, DateTime CreatedAt, DateTime UpdatedAt);
public record CreateNoteRequest(string? Title, string? Content, int? FolderId);
public record UpdateNoteRequest(string? Title, string? Content, int? FolderId);
public record NoteResponse(string Message, NoteDto? Note = null);
public record NotesResponse(List<NoteDto> Notes);
public record ShareResponse(string Message, string ShareUrl);
public record VersionsResponse(List<NoteVersionDto> Versions);

// Folder DTOs
public record FolderDto(int Id, string Name, int NoteCount);
